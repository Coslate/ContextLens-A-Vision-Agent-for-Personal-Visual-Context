...
| Linking pairwise F1 | 0.333 | 0.000 |
...
- linking is present but still weak (`pairwise F1 = 0.333`).
...
- **Cross-image linking where the baseline has none** (3 groups, F1 0.333 vs 0 groups) — related images are connected into higher-level context
...
The system is honest about its current limitations — adversarial extraction is still challenging (0.357), failure flag coverage is incomplete (3/7), and linking is present but weak (F1 0.333). These limitations are not hidden; they are quantified, reported, and directly motivate the next steps outlined in Section 14.
